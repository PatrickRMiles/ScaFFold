...
class BaseTrainer:
    ...
    def create_dataloaders(self):
        ...
        if num_workers > 0:
            loader_args["prefetch_factor"] = max(
                1, int(self.config.dataloader_prefetch_factor)
            )
        self.log.debug(
            f"dataloader num_workers={loader_args['num_workers']}, prefetch_factor={loader_args.get('prefetch_factor', 'n/a')}, persistent_workers={loader_args['persistent_workers']}, os.cpu_count()={os.cpu_count()}, self.world_size={self.world_size} "
        )
